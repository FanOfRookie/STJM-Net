...
gruop_label = ['STJM Net', 'MS-G3D']
gruop_color = ['green', 'blue']
test = np.ones(60)
...
ax.legend(fontsize=24,ncol = gruop_label.__len__(),loc='upper left')
plt.xticks(x + 2, actions)
# 设置x轴刻度文字竖排显示
plt.xticks(fontsize=30)
plt.setp(plt.gca().get_xticklabels(), rotation=70, ha='right')
plt.tight_layout()
plt.yticks(fontsize=30)
plt.savefig('per-class_acc.png')
plt.show()